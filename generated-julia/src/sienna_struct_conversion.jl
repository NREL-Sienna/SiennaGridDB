...
function get_function_data(::T) where {T <: PSY.LinearFunctionData}
	error("Unsupported type $T")
end
...
sys = PowerSystemCaseBuilder.build_system(PowerSystemCaseBuilder.PSISystems, "c_sys5_pjm")

thermal_standard = PSY.get_component(PSY.ThermalStandard, sys, "Solitude")

test_convert = convert(thermal_standard)
post_json = OpenAPI.from_json(ThermalStandard, JSON.parse(OpenAPI.to_json(test_convert)))
@assert test_convert == post_json "Conversion test failed"
