# This file was generated by the Julia OpenAPI Code Generator
# Do not modify this file directly. Modify the OpenAPI specification instead.

include("models/model_ACBus.jl")
include("models/model_AGC.jl")
include("models/model_Arc.jl")
include("models/model_Area.jl")
include("models/model_AreaInterchange.jl")
include("models/model_AverageRateCurve.jl")
include("models/model_AverageRateCurveFunctionData.jl")
include("models/model_ConstantReserve.jl")
include("models/model_ConstantReserveGroup.jl")
include("models/model_ConstantReserveNonSpinning.jl")
include("models/model_CostCurve.jl")
include("models/model_CostCurve1.jl")
include("models/model_CostCurveValueCurve.jl")
include("models/model_DCBus.jl")
include("models/model_EnergyReservoirStorage.jl")
include("models/model_EnergyReservoirStorageEfficiency.jl")
include("models/model_EnergyReservoirStorageOperationCost.jl")
include("models/model_EnergyReservoirStorageOperationCostChargeVariableCost.jl")
include("models/model_EnergyReservoirStorageOperationCostStartUp.jl")
include("models/model_EnergyReservoirStorageOperationCostStartUpOneOf.jl")
include("models/model_ExponentialLoad.jl")
include("models/model_FixedAdmittance.jl")
include("models/model_FixedAdmittanceY.jl")
include("models/model_FromTo.jl")
include("models/model_FromToToFrom.jl")
include("models/model_FuelCurve.jl")
include("models/model_FuelCurveFuelCost.jl")
include("models/model_HydroDispatch.jl")
include("models/model_HydroDispatchOperationCost.jl")
include("models/model_HydroEnergyReservoir.jl")
include("models/model_HydroEnergyReservoirOperationCost.jl")
include("models/model_HydroPumpedStorage.jl")
include("models/model_IncrementalCurve.jl")
include("models/model_IncrementalCurveFunctionData.jl")
include("models/model_InputOutputCurve.jl")
include("models/model_InputOutputCurveFunctionData.jl")
include("models/model_InterconnectingConverter.jl")
include("models/model_InterconnectingConverterLossFunction.jl")
include("models/model_InterruptiblePowerLoad.jl")
include("models/model_InterruptiblePowerLoadOperationCost.jl")
include("models/model_Line.jl")
include("models/model_LineArc.jl")
include("models/model_LinearCurve.jl")
include("models/model_LinearFunctionData.jl")
include("models/model_LoadZone.jl")
include("models/model_MinMax.jl")
include("models/model_MonitoredLine.jl")
include("models/model_PhaseShiftingTransformer.jl")
include("models/model_PiecewiseLinearData.jl")
include("models/model_PiecewiseLinearDataPointsInner.jl")
include("models/model_PiecewiseStepData.jl")
include("models/model_PowerLoad.jl")
include("models/model_ProductionVariableCostCurve.jl")
include("models/model_QuadraticCurve.jl")
include("models/model_QuadraticFunctionData.jl")
include("models/model_RenewableDispatch.jl")
include("models/model_RenewableDispatchOperationCost.jl")
include("models/model_RenewableiNonDispatch.jl")
include("models/model_Source.jl")
include("models/model_StandardLoad.jl")
include("models/model_StartUpShutDown.jl")
include("models/model_StartUpStages.jl")
include("models/model_SwitchedAdmittance.jl")
include("models/model_TModelHVDCLine.jl")
include("models/model_TapTransformer.jl")
include("models/model_ThermalGenerationCost.jl")
include("models/model_ThermalGenerationCostStartUp.jl")
include("models/model_ThermalMultiStart.jl")
include("models/model_ThermalStandard.jl")
include("models/model_Transformer2W.jl")
include("models/model_TwoTerminalVSCDCLine.jl")
include("models/model_UpDown.jl")
include("models/model_VariableReserve.jl")
include("models/model_VariableReserveNonSpinning.jl")
