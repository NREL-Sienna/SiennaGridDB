...
Base.@kwdef mutable struct LinearCurve <: OpenAPI.APIModel
    curve_type::Union{Nothing, String} = "LINEAR"
    marginal_cost::Union{Nothing, Float64} = nothing
    no_load_cost::Union{Nothing, Float64} = 0.0

    function LinearCurve(curve_type, marginal_cost, no_load_cost)
        OpenAPI.validate_property(LinearCurve, Symbol("curve_type"), curve_type)
        ...
        return new(curve_type, marginal_cost, no_load_cost)
    end
...
function OpenAPI.validate_property(::Type{LinearCurve}, name::Symbol, val)
    if name === Symbol("curve_type")
        OpenAPI.validate_param(name, "LinearCurve", :enum, val, ["LINEAR"])
    end
end
