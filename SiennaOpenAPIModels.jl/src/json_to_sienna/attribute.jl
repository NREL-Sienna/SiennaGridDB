...
function openapi2psip(attribute::RetrofitPotential, resolver::Resolver)
    PSIP.RetrofitPotential(; eligible_generators=attribute.eligible_generators)
end

function openapi2psip(attribute::AggregateRetirementPotential, resolver::Resolver)
    PSIP.AggregateRetirementPotential(; retirement_potential=attribute.retirement_potential)
end
...
function openapi2psip(attribute::TopologyMapping, resolver::Resolver)
    PSIP.TopologyMapping(; buses=attribute.buses)
end

function openapi2psip(attribute::ExistingCapacity, resolver::Resolver)
    PSIP.ExistingCapacity(; existing_technologies=attribute.existing_technologies)
end
