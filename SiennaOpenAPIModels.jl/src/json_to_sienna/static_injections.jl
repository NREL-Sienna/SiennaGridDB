function openapi2psy(thermal::ThermalStandard, resolver::Resolver)
    ...
    PSY.ThermalStandard(;
        ...
        active_power_limits=get_tuple_min_max(
            divide(thermal.active_power_limits, thermal.base_power),
        ),
        reactive_power_limits=get_tuple_min_max(
            divide(thermal.reactive_power_limits, thermal.base_power),
        ),
        ramp_limits=get_tuple_up_down(divide(thermal.ramp_limits, thermal.base_power)),
