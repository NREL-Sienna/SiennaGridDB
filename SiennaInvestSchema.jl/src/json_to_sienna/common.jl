...
function get_sienna_technology_financial_data(financial_data::TechnologyFinancialData) #This is actually not in openAPI, let's fix this
...
function get_sienna_fuel_dictionary(fuels::Dict)
    fuel_dictionary = Dict{PSY.ThermalFuels, Any}()
    for (fuel, value) in fuels
        fuel_enum = get_fuel_type_enum(fuel)
        if isa(value, Dict)
            fuel_dictionary[fuel_enum] = get_tuple_min_max(value)
        else
            fuel_dictionary[fuel_enum] = value
        end
    end
